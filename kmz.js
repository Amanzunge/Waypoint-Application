...
function generatePlacemark(waypoint, nextWaypoint = null) {
  ...
  if (nextWaypoint) {
    ...
    if (nextCoords) heading = calculateHeading(coords, nextCoords);
  }
  ...
  switch (waypoint.action) {
    case 'take_picture':
      actionTag = `
        <wpml:action>
          <wpml:actionId>${1000 + waypoint.order}</wpml:actionId>
          <wpml:actionActuatorFunc>cameraTakePhoto</wpml:actionActuatorFunc>
        </wpml:action>`;
      break;
...
<Placemark>
  ...
  <wpml:index>${waypoint.order}</wpml:index>
  ...
  <wpml:actionGroup>
    <wpml:actionGroupId>${waypoint.order}</wpml:actionGroupId>
    <wpml:actionGroupStartIndex>${waypoint.order}</wpml:actionGroupStartIndex>
    <wpml:actionGroupEndIndex>${waypoint.order}</wpml:actionGroupEndIndex>
    ...
    ${actionTag}
    <wpml:action>
      <wpml:actionId>${2000 + waypoint.order}</wpml:actionId>
      ...
      JSZip.loadAsync(e.target.result)
        ...
        .then(wpmlContent => {
          ...
          for (let i = 0; i < placemarks.length; i++) {
            ...
            // Detect custom camera/video action
            const actionElements = placemark.getElementsByTagNameNS("*", "actionActuatorFunc");
            let action = 'no_action';
            for (let i = 0; i < actionElements.length; i++) {
            const func = actionElements[i].textContent.trim();
            if (func === 'cameraTakePhoto') action = 'take_picture';
            else if (func === 'startRecord') action = 'start_recording';
            else if (func === 'stopRecord') action = 'stop_recording';
            }
            ...
            const marker = L.marker(latlng, {
              ...
              .bindPopup(createPopupContent(latlng, altitude, speed, gimbal, order, action));
